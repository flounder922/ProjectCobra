...
public class MoveBackwardAction extends AbstractInputAction {
    ...
    public MoveBackwardAction(Camera camera, SceneNode node) {
        this.camera = camera;
        this.node = node;
    }
    ...
    public void performAction(float time, Event event) {
        ...
        if (camera.getMode() == 'c') {
            Vector3f v = camera.getFd(); // Gets the current forward vector of the camera and stores it in v.
            ...
            node.moveBackward(0.025f);

        } else {
            System.out.println("I Did something wrong in Move Backwards Action");
        }
    }
}
